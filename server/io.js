    ...
    socket.on('message',async (from, to,message,type)=> {
      if(to.type==='user'){
        const privateMsg={
          ...
          isRecalled:false,
          _id:'private_'+socket.user.id+'_'+to.id+'_'+new Date().getTime()
        };
        socket.emit('message-id', privateMsg._id, to.id);
        socket.broadcast.to(to.roomId).emit('message', socket.user, to, message, type, privateMsg._id);
      }
      if(to.type==='group'){
        const savedMsg = await store.saveMessage(from,to,message,type);
        socket.emit('message-id', savedMsg._id, to.id);
        socket.broadcast.emit('message', socket.user, to, message, type, savedMsg._id);
      }
    });
    socket.on('recall-message',async (messageId, session)=> {
      try {
        let message = null;
        if(session.type === 'group') {
          message = await store.getMessageById(messageId);
        } else {
          message = {
            _id: messageId,
            from: socket.user,
            to: session,
            isRecalled: false,
            time: parseInt(messageId.split('_').pop())
          };
        }
        
        if(!store.canRecallMessage(message, socket.user.id)) {
          socket.emit('recall-fail', messageId, '无法撤回该消息');
          return;
        }
        
        if(session.type === 'group') {
          await store.recallMessage(messageId);
          socket.broadcast.emit('recall-message', messageId, session);
        } else {
          socket.broadcast.to(session.roomId).emit('recall-message', messageId, session);
        }
        
        socket.emit('recall-success', messageId, session);
        console.log(`用户<${socket.user.name}>撤回了消息`);
      } catch (err) {
        console.error('撤回消息失败:', err);
        socket.emit('recall-fail', messageId, '撤回失败，请稍后重试');
      }
    });
